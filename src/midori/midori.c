...
void midori_key_generation(const uint8_t *key, uint8_t *round_keys) {
    ...
    // 082e fa98 ec4e 6c89
    // 4528 21e6 38d0
    uint8_t roundConstant[38] = {0x24, 0x3f, 0x6a, 0x88, 0x85, 0xa3, 0x08, 0xd3, 0x13, 0x19, 0x8a, 0x2e, 0x03, 0x70,
                                 0x73, 0x44, 0xa4, 0x09, 0x38, 0x22, 0x29, 0x9f, 0x31, 0xd0, 0x08, 0x2e, 0xfa, 0x98,
                                 0xec, 0x4e, 0x6c, 0x89, 0x45, 0x28, 0x21, 0xe6, 0x38, 0xd0};

    for (int i = 0; i < 19; ++i) {
        for (int j = 0; j < 14; ++j) {
            round_keys[j + i * 16] = key[j];
        }
        round_keys[14 + i * 16] = key[14] ^ roundConstant[0 + i * 2];
        round_keys[15 + i * 16] = key[15] ^ roundConstant[1 + i * 2];
    }
